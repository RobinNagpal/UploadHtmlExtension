...
let EMBEDDED_IMAGE_BUTTON_MESSAGE,
  SHARE_PAGE_BUTTON_MESSAGE,
  SHARE_SELECTION_BUTTON_MESSAGE,
  ERROR_TITLE_MESSAGE;

try {
  EMBEDDED_IMAGE_BUTTON_MESSAGE = browser.i18n.getMessage(
    "topPanelEmbeddedImageButton"
  );
  SHARE_PAGE_BUTTON_MESSAGE = browser.i18n.getMessage(
    "topPanelSharePageButton"
  );
  SHARE_SELECTION_BUTTON_MESSAGE = browser.i18n.getMessage(
...
export { downloadPage, downloadPageForeground };
...
async function downloadPage(pageData, options) {
  ...
  if (options.compressContent) {
    ...
    if (pageData.filename) {
      const blob = new Blob([await yabson.serialize(pageData)], {
        type: pageData.mimeType,
      });
...
async function sharePage(pageData, options) {
  ...
  if (!cancelled) {
    const data = {
      files: [
        new File([pageData.content], pageData.filename, {
          type: pageData.mimeType,
        }),
      ],
    };
