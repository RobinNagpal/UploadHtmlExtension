...
const SHARE_PAGE_BUTTON_MESSAGE = browser.i18n.getMessage(
  "topPanelSharePageButton"
);
const SHARE_SELECTION_BUTTON_MESSAGE = browser.i18n.getMessage(
...
async function processPage(options) {
  ...
  options.onprogress = async (event) => {
    ...
    if (!processor.cancelled) {
      if (event.type == event.RESOURCES_INITIALIZING) {
        if (
          !initializing &&
          options.insertEmbeddedScreenshotImage &&
          options.compressContent
        ) {
          initializing = true;
          ui.setVisible(false);
  ...
  if (shouldShowModal) {
    await takeInputsFromUser((capture) => {
      console.log("Callback function called");
      if (capture) {
        console.log("Capture is true");
      } else {
        console.log("Capture is false");
      }
    });
    // return;
  }
